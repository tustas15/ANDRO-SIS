...
{
    // Validar los datos antes de guardar (ajusta según tus reglas)
    if ($this->validate($this->proyectoModel->validationRules)) {
        // Convertir fechas al formato MySQL
        $fechaPublicacion = date('Y-m-d H:i:s', strtotime($this->request->getPost('fecha_publicacion')));
        $fechaFin = $this->request->getPost('fecha_fin') 
            ? date('Y-m-d H:i:s', strtotime($this->request->getPost('fecha_fin'))) 
            : null;

        // Guardar proyecto con las fechas
        $this->proyectoModel->save([
            ...
            'fecha_publicacion' => $fechaPublicacion, // Nueva fecha desde el formulario
            'fecha_fin' => $fechaFin // Fecha opcional
        ]);
    ...
    public function update()
{
    ...
    // Convertir fechas al formato MySQL
    $data['fecha_publicacion'] = date('Y-m-d H:i:s', strtotime($data['fecha_publicacion']));
    $data['fecha_fin'] = !empty($data['fecha_fin']) 
        ? date('Y-m-d H:i:s', strtotime($data['fecha_fin'])) 
        : null;
